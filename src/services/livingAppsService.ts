...
import type { Personenpunkte, CreatePersonenpunkte } from '@/types/app';
...
export class LivingAppsService {
  // --- PERSONENPUNKTE ---
  static async getPersonenpunkte(): Promise<Personenpunkte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.PERSONENPUNKTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Personenpunkte[];
    return enrichLookupFields(records, 'personenpunkte');
  }
  static async getPersonenpunkteEntry(id: string): Promise<Personenpunkte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.PERSONENPUNKTE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Personenpunkte;
    return enrichLookupFields([record], 'personenpunkte')[0];
  }
  static async createPersonenpunkteEntry(fields: CreatePersonenpunkte) {
    return callApi('POST', `/apps/${APP_IDS.PERSONENPUNKTE}/records`, { fields: cleanFieldsForApi(fields as any, 'personenpunkte') });
  }
  static async updatePersonenpunkteEntry(id: string, fields: Partial<CreatePersonenpunkte>) {
    return callApi('PATCH', `/apps/${APP_IDS.PERSONENPUNKTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'personenpunkte') });
  }
  static async deletePersonenpunkteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.PERSONENPUNKTE}/records/${id}`);
  }
