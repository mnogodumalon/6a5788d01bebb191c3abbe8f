...
import { PersonenpunkteViewDialog } from '@/components/dialogs/PersonenpunkteViewDialog';
...
const PERSONENPUNKTE_FIELDS = [
  ...
  { key: 'kategorie', label: 'Kategorie', type: 'lookup/select', options: [{ key: 'bronze', label: 'Bronze' }, { key: 'silber', label: 'Silber' }, { key: 'gold', label: 'Gold' }, { key: 'platin', label: 'Platin' }] },
  { key: 'punkte', label: 'Punktestand', type: 'number' },
  { key: 'bemerkung', label: 'Bemerkung', type: 'string/textarea' },
];

const ENTITY_TABS = [
  { key: 'personenpunkte', label: 'Personenpunkte', pascal: 'Personenpunkte' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('personenpunkte');
  const [selectedIds, setSelectedIds] = useState<Record<EntityKey, Set<string>>>(() => ({
    'personenpunkte': new Set(),
  }));
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'personenpunkte': return (data as any).personenpunkte as Personenpunkte[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'personenpunkte': return PERSONENPUNKTE_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'personenpunkte': return {
        create: (fields: any) => LivingAppsService.createPersonenpunkteEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updatePersonenpunkteEntry(id, fields),
        remove: (id: string) => LivingAppsService.deletePersonenpunkteEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'personenpunkte' || dialogState?.entity === 'personenpunkte') && (
        <PersonenpunkteDialog
          open={createEntity === 'personenpunkte' || dialogState?.entity === 'personenpunkte'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'personenpunkte' ? handleUpdate : (fields: any) => handleCreate('personenpunkte', fields)}
          defaultValues={dialogState?.entity === 'personenpunkte' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Personenpunkte']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Personenpunkte']}
        />
      )}
      {viewState?.entity === 'personenpunkte' && (
        <PersonenpunkteViewDialog
          open={viewState?.entity === 'personenpunkte'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'personenpunkte', record: r }); }}
        />
      )}
