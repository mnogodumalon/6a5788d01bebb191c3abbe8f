...
interface PersonenpunkteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Personenpunkte['fields']) => Promise<void>;
  defaultValues?: Personenpunkte['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function PersonenpunkteDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: PersonenpunkteDialogProps) {
  const [fields, setFields] = useState<Partial<Personenpunkte['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'personenpunkte');
      await onSubmit(clean as Personenpunkte['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "vorname": string | null, // Vorname\n  "nachname": string | null, // Nachname\n  "email": string | null, // E-Mail-Adresse\n  "kategorie": LookupValue | null, // Kategorie (select one key: "bronze" | "silber" | "gold" | "platin") mapping: bronze=Bronze, silber=Silber, gold=Gold, platin=Platin\n  "punkte": number | null, // Punktestand\n  "bemerkung": string | null, // Bemerkung\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Personenpunkte['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Personenpunkte bearbeiten' : 'Personenpunkte hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="punkte">Punktestand</Label>
            <Input
              id="punkte"
              type="number"
              value={fields.punkte ?? ''}
              onChange={e => setFields(f => ({ ...f, punkte: e.target.value ? Number(e.target.value) : undefined }))}
            />
          </div>
          <div className="space-y-2">
